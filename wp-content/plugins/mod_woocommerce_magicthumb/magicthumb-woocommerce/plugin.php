...
function WooCommerce_MagicThumb_send_stat($action = '') {
    ...
    if('working' == 'v5.12.24' || 'working' == 'v2.0.69') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magicthumb&license=trial&tool_version=v2.0.69&module_version=v5.12.24&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
...
function  magictoolbox_WooCommerce_MagicThumb_callback($matches) {
    ...
    $result = preg_replace("/^(.*?)<img(.*?)$/is","$1<img {$matches[2]}$2",$result);
     ...
     if ($is_main) {
        ...
        if (!$cat) {
            if ($GLOBALS['MAGICTOOLBOX_'.strtoupper('magicthumb').'_MAIN_IMAGE_SET']) $result = $matches[0];
...
function WooCommerce_MagicThumb_get_post_attachments()  {
	...
	if ($mainImage->guid != $attachments[0]->guid) {
