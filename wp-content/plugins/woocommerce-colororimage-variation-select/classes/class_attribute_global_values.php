	...
	wp_enqueue_style( 'wp-color-picker' );
		...
		<div class="" >
			...
			<div style="line-height:60px;">
				...
				<button type="button" class="wcva_upload_image_button button"><?php _e( 'Upload/Add image', 'wcva' ); ?></button>
				<button type="button" class="wcva_remove_image_button button"><?php _e( 'Remove image', 'wcva' ); ?></button>
			</div>
			<script type="text/javascript">

				 // Only show the "remove image" button when needed
				 if ( ! jQuery('#thumbnail_id').val() )
					 jQuery('.wcva_remove_image_button').hide();

				// Uploading files
				var file_frame;

				jQuery(document).on( 'click', '.wcva_upload_image_button', function( event ){
					...
					file_frame.on( 'select', function() {
						...
						jQuery('.wcva_remove_image_button').show();
					});
				...
				jQuery(document).on( 'click', '.wcva_remove_image_button', function( event ){
		...
		<tr class="form-field" id="wcvaimagep" style="<?php if (isset($display_type) && ($display_type == "Image")) { echo 'display:;'; } else { echo 'display:none;'; }  ?>">
			...
			<td>
				...
				<div style="line-height:60px;">
					...
					<button type="submit" class="wcva_upload_image_button button"><?php _e( 'Upload/Add image', 'wcva' ); ?></button>
					<button type="submit" class="wcva_remove_image_button button"><?php _e( 'Remove image', 'wcva' ); ?></button>
				</div>
				<script type="text/javascript">
