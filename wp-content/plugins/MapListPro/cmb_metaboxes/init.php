...
class cmb_Meta_Box {
	...
	/**
	 * This will add place locator to multisite by inserting post
	 */
	public static function add_post_to_multisite($data)
	{
		$post_title = sanitize_text_field( $data['post_title'] );
		$user_ID = $data['user_ID'];
		
		$meta_data = array(
			"maplist_latitude" => $data['maplist_latitude'],
			"maplist_longitude" => $data['maplist_longitude'],
			"maplist_description" => $data['maplist_description'],
			"maplist_address" => $data['maplist_address'],
			"maplist_alternateurl" => $data['maplist_alternateurl'],
			"maplist_marker" => $data['maplist_marker'],
			"maplist_hours" => $data['maplist_hours'],
			"store_parent_id" => $data['ID']
		);
		...
		if($multisites)
		{			
			$terms = $data['tax_input']['map_location_categories'];
			...
			foreach($multisites as $multisite)
			{
				...
				//wp_set_post_categories( $new_post_id, array(112, 113), true );
				...
				update_post_meta( $data['ID'], 'blog_id_'.$blog_id, $new_post_id );
			}	
		}
	}
	/**
	 * This will add place locator to multisite by inserting post
	 */
	public static function update_post_to_multisite($data)
	{
		$post_title = sanitize_text_field( $data['post_title'] );
		//$user_ID = $data['user_ID'];
		...
		if($multisites)
		{
			...
			foreach($multisites as $multisite)
			{
				...
				$current_post_id = get_post_meta( $data['ID'], 'blog_id_'.$blog_id, true );
				...
				/*$my_post = array(
				  ...
				  'post_status'   => 'publish',
				  'post_author'   => $user_ID,
				  'post_category' => $data['tax_input']['map_location_categories'],
				  'post_type'	  => 'maplist'
				);
				// Insert the post into the database
				$new_post_id = wp_insert_post( $my_post );*/
				...
				wp_delete_object_term_relationships( $current_post_id, 'map_location_categories' );
				...
				foreach($meta_data as $key => $val)
				{
					update_post_meta( $current_post_id, $key, $val );
				}
	...
	public function save_post( $post_id, $post = false ) {
		...
		// If store is updated
		if($post_type == 'maplist' && isset($_POST['save']) && $_POST['save'] == 'Update')
		{
			remove_action( 'save_post', array( $this, 'save_post' ), 10, 2 );
			self::update_post_to_multisite($_POST);
			add_action( 'save_post', array( $this, 'save_post' ), 10, 2 );	
		}
		else
		{
			remove_action( 'save_post', array( $this, 'save_post' ), 10, 2 );
			self::add_post_to_multisite($_POST);
			add_action( 'save_post', array( $this, 'save_post' ), 10, 2 );
		}
	}
	/**
